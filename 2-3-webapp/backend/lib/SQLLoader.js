const fs = require('fs');
const QUERIES_DIR = fs.realpathSync('./db/queries/');
...
class SQLLoader {
			...
			readQueries(QUERIES_DIR)
				.then(queries => {
					queries.forEach(query => {
						self.queries[query.filename.replace(/\.sql$/, '')] = query.content;
					});
	...
	loadQuerySync(queryName) {
		try {
			...
			throw new Error(`Can't find query "${queryName}.sql" in the queries folder (${QUERIES_DIR})`);
		}
	}
}
